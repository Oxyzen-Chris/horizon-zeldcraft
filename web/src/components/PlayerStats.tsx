...
/**
 * Statistiques de progression pour un joueur donné, requêtées on-chain :
 * voxlynOf → voxlyns → questCompleted[] + npcMet[] + treasureFound[] + worldUnlocked[] + playerScore.
 */
export function PlayerStats({ contract }: { contract: `0x${string}` }) {
  ...
  const load = () => {
    if (isAddress(addr)) setTarget(addr as `0x${string}`);
