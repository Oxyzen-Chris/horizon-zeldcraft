...
export function Scoreboard({ contract, tokenId, level, xp, offchainScore = 0 }: {
  contract: `0x${string}`; tokenId: bigint; level: number; xp: number; offchainScore?: number;
}) {
  const { t } = useI18n();
  ...
  const totalScore = Number(score ?? 0) + offchainScore;
  return (
    <div className="card">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Stat label={t('game.scoreboard.score')} value={totalScore}       color="text-yellow-400" />
