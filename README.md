...
| Plateforme 3D : jambes/pattes des PNJ (et dans une moindre mesure des familiers/dragons) visiblement enfoncées dans la dalle de terrain, contrairement à Synk toujours correctement posé au sol | Le relevage anti-enterrement ne compensait que le surplus d'enfoncement dû à une échelle `>1×` (nul par construction à l'échelle par défaut des PNJ) — les personnages vivants (PNJ/familier/faune errante) reçoivent désormais un calage FIXE égal à l'enfoncement géométrique **complet**, sans plus jamais léviter comme un objet magique (voir `docs/ARCHITECTURE.md` § PNJ (et familiers/faune) : jambes/pattes enfoncées dans le sol) |
| Plateforme 3D : régression du correctif ci-dessus — le loup-garou (et la faune errante en général : hibou, sanglier) flottait bien trop haut au-dessus du sol | Le calage FIXE (`groundAnchorUnscaled(0.39) * scale`) calculé pour la géométrie profonde de `NpcVoxel`/`DragonMarker` avait été appliqué à tort à la faune errante, dont les modèles dédiés (`Owl3D`/`Werewolf3D`/`Boar3D`) ont des pattes bien plus proches du sol et gèrent déjà leur propre élévation en interne — `isWildlife` retiré du mécanisme, la faune retrouve exactement son léger flottement naturel d'origine (voir `docs/ARCHITECTURE.md` § Régression : la faune errante flottait trop haut au-dessus du sol) |
| Plateforme 3D : le loup-garou/sangliers/marcassins ne touchaient toujours pas tout à fait le sol, et sangliers/marcassins « rebondissaient » sur place comme un objet de quête | La faune retombait dans la branche des objets EN LÉVITATION (`bobAmplitude(0.15)` + oscillation, qui ne s'annule jamais : `0.09` à `0.21`) — elle rejoint désormais `isLivingCharacter` (comme PNJ/familier) avec un calage FIXE (zéro oscillation) calibré **par sous-espèce** selon sa propre géométrie (`0.06` loup-garou, `0` hibou/sanglier, déjà au niveau du sol par construction) (voir `docs/ARCHITECTURE.md` § La faune errante toujours flottante et rebondissante : calage fixe PAR SOUS-ESPÈCE) |
| PNJ/familiers/faune errante (loup-garou, sangliers, marcassins…) traversaient librement l'eau, les montagnes, les arbres, huttes et châteaux au lieu de les contourner | `lib/roamingActors.ts::advanceActor()` (moteur PARTAGÉ 2D/3D/Mapmonde) choisissait une direction purement aléatoire sans conscience du terrain — vérifie désormais chaque case cible via `isTileBlockedForRoaming()` (eau/rocher/props solides/POI obstacle) et recherche une direction de contournement (`findDetourDirection`) si bloquée ; paramétrable en Administration (`RepRules.roamObstacleAvoidanceEnabled`, défaut activé) (voir `docs/ARCHITECTURE.md` § Évitement intelligent des obstacles par les PNJ/familiers/faune errants) |
| PNJ, familiers et faune errante (loup-garou, hiboux, dragons, sangliers…) se traversaient entre eux et traversaient Synk lui-même au lieu de se contourner | Aucun contrôle mutuel entre acteurs — ajout de `isTileBlockedByOtherActor()` (distance réelle `< ACTOR_COLLISION_RADIUS` à tout autre acteur vivant ou à Synk, combiné à l'évitement de terrain existant), d'une résolution rétroactive des collisions simultanées (`resolveSimultaneousCollisions`) et d'un anti-chevauchement dès le spawn de la faune (`randomWildlifeSpawnAvoidingOverlap`) ; paramétrable en Administration (`RepRules.roamActorCollisionEnabled`, défaut activé, indépendant de l'évitement de terrain) (voir `docs/ARCHITECTURE.md` § Évitement mutuel entre acteurs errants) |
| Hiboux/sangliers/marcassins traversaient toujours visuellement les murs de château/hutte ; sanglier/marcassin de nouveau « aimanté » à Synk (translation latérale calquée sur ses mouvements) | (1) Seule la case EXACTE d'un château/hutte était bloquée alors que sa silhouette 3D déborde d'~0,3-0,5 unité sur les cases voisines — nouvelle `isNearBlockingPropFootprint()` (rayon d'emprise par prop) + anti-spawn dans un obstacle + correction d'un bord de mapmonde qui court-circuitait le contrôle d'obstacle. (2) Synk était traité comme un autre acteur (contournement actif) dans le cas marginal post-délai de grâce du gel de proximité — `isBlockedBySynkProximity()` dédiée fait désormais arrêter l'acteur sur place au lieu de tenter un contournement actif, supprimant la boucle de rétroaction responsable de l'effet d'orbite (voir `docs/ARCHITECTURE.md` § Régression #7) |
| Saturation anormale du GPU (~97-100% du GPU intégré, ventilateurs qui s'emballent) à l'ouverture du jeu dans le navigateur, sur plusieurs machines différentes — persiste après le correctif `powerPreference` initial ET après la procédure Windows de réassignation du GPU par exécutable | `powerPreference: 'high-performance'` + `stencil: false` + ombres/anticrénelage désactivables déployés ET vérifiés actifs en production ; `frameloop` suspendu hors premier plan. **La capture d'écran des Paramètres Windows fournie montre que la « Préférence GPU » de `msedge.exe` affiche toujours « Économie d'énergie »** malgré la manipulation signalée — à re-sélectionner explicitement en « Performances élevées » avant toute autre piste (voir `docs/ARCHITECTURE.md` § Suite 2). Investigation du commit signalé comme point de départ (`2ee6e47`, boussole) : aucun nouveau coût de rendu continu identifié hormis un `backdrop-blur-sm` (flou CSS posé en permanence sur le canvas WebGL en rendu continu, coût de compositing connu sur Chromium) — retiré par précaution (remplacé par un fond opaque, purement cosmétique, sans impact gameplay) |
