...
| Saturation anormale du GPU (~97-100% du GPU intégré, ventilateurs qui s'emballent) à l'ouverture du jeu dans le navigateur, sur plusieurs machines différentes | Le `<Canvas>` React Three Fiber de la Plateforme 3D ne passait aucun indice `powerPreference` au contexte WebGL, laissant le navigateur/l'OS router le rendu vers le GPU intégré basse consommation plutôt que le GPU dédié — ajout de `powerPreference: 'high-performance'` (paramétrable en Administration, `platform3dHighPerformanceGpuEnabled`) + `stencil: false` (tampon inutilisé) + ombres portées rendues désactivables (`platform3dShadowsEnabled`) en dépannage. Aucun changement visuel par défaut, vérifié via Playwright (`getContextAttributes()`) (voir `docs/ARCHITECTURE.md` § Optimisation GPU/CPU) |
