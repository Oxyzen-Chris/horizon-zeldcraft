...
| Saturation anormale du GPU (~97-100% du GPU intégré, ventilateurs qui s'emballent) à l'ouverture du jeu dans le navigateur, sur plusieurs machines différentes — persiste après le correctif `powerPreference` initial, y compris widget 3D fermé | `powerPreference: 'high-performance'` (paramétrable, `platform3dHighPerformanceGpuEnabled`) + `stencil: false` + ombres désactivables (`platform3dShadowsEnabled`) déployés ET vérifiés actifs en production, mais Windows peut **verrouiller le GPU par exécutable navigateur** au niveau OS (Paramètres → Affichage → Graphismes), ce qu'aucun indice JS ne peut contourner — **procédure Windows définitive documentée** (voir `docs/ARCHITECTURE.md` § Suite : `powerPreference`... ). Compléments code, vérifiés Playwright, sans régression : rendu totalement suspendu quand l'onglet n'est pas au premier plan (`frameloop: 'never'` piloté par `visibilitychange`, 0 appel `drawElements` onglet masqué, reprise instantanée) + anticrénelage désactivable (`platform3dAntialiasEnabled`) |
