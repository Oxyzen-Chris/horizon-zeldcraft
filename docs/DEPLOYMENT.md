...
| `GEMINI_API_KEY`                            | ⚠️     | Optionnelle — active l'assistant IA du menu Admin « Intelligence IA GamePlay » (fournisseur « Google Gemini »). **Clé serveur uniquement** (pas de préfixe `NEXT_PUBLIC_`, jamais exposée au navigateur). Génère-la gratuitement sur https://aistudio.google.com/app/apikey (quota gratuit sans carte bancaire). |
| `GROQ_API_KEY`                               | ⚠️     | Optionnelle — active le fournisseur IA « Groq » (endpoint compatible OpenAI, modèle par défaut `llama-3.3-70b-versatile`). **Clé serveur uniquement**. Génère-la gratuitement sur https://console.groq.com/keys (quota gratuit très généreux, sans carte bancaire). |
| `CEREBRAS_API_KEY`                           | ⚠️     | Optionnelle — active le fournisseur IA « Cerebras » (endpoint compatible OpenAI, modèle par défaut `llama-3.3-70b`). **Clé serveur uniquement**. Génère-la gratuitement sur https://cloud.cerebras.ai (quota gratuit généreux, sans carte bancaire). |
| `OPENROUTER_API_KEY`                         | ⚠️     | Optionnelle — active le fournisseur IA « OpenRouter » (endpoint compatible OpenAI, modèle par défaut `meta-llama/llama-3.3-70b-instruct:free`, un modèle au suffixe `:free`, donc 100% gratuit). **Clé serveur uniquement**. Génère-la gratuitement sur https://openrouter.ai/keys (aucune carte bancaire requise). |
| `RESEND_API_KEY`                             | ⚠️     | Optionnelle — active l'envoi d'e-mails (bienvenue, rapports de progression, annonces admin — voir `docs/EMAIL_NOTIFICATIONS.md`). **Clé serveur uniquement**. Génère-la gratuitement sur https://resend.com/api-keys (3000 e-mails/mois gratuits, sans carte bancaire). |
| `RESEND_FROM_EMAIL`                          | ⚠️     | Adresse d'expédition des e-mails du jeu. Utilise `onboarding@resend.dev` (adresse de test Resend) tant qu'aucun domaine n'est vérifié dans https://resend.com/domains, puis ta propre adresse (ex. `jeu@tondomaine.fr`) une fois le domaine vérifié. |
| `RESEND_FROM_NAME`                           | ⚠️     | Nom d'expéditeur affiché (ex. `Horizon ZeldCraft`). Défaut si absent : `Horizon ZeldCraft`. |
| `FIREBASE_DB_SECRET`                          | ⚠️     | Optionnelle — nécessaire UNIQUEMENT pour les rapports de progression **programmés** (envoi cron automatique, voir `docs/EMAIL_NOTIFICATIONS.md`). Firebase Console → ⚙️ Project settings → Comptes de service → onglet « Secrets de base de données (legacy) » → Générer. **Clé serveur uniquement**, ne jamais exposer côté client (elle contourne toutes les règles de sécurité RTDB). |
| `CRON_SECRET`                                 | ⚠️     | Optionnelle — sécurise l'endpoint `/api/email/cron-reports` (Vercel Cron l'envoie automatiquement en en-tête `Authorization: Bearer <CRON_SECRET>` dès qu'elle est définie). Génère une valeur aléatoire longue, ex. `openssl rand -hex 32`. |
| `FIREBASE_ADMIN_CLIENT_EMAIL`                 | ⚠️     | Optionnelle — nécessaire UNIQUEMENT pour le bouton admin « 🔑 Reset mot de passe » (rubrique « Statistiques par joueur » → Zone de danger, voir `docs/EMAIL_NOTIFICATIONS.md`). Firebase Console → ⚙️ Project settings → Comptes de service → « Générer une nouvelle clé privée » → champ `client_email` du fichier JSON téléchargé. **Clé serveur uniquement**. |
| `FIREBASE_ADMIN_PRIVATE_KEY`                  | ⚠️     | Optionnelle — accompagne `FIREBASE_ADMIN_CLIENT_EMAIL` ci-dessus. Champ `private_key` du même fichier JSON (colle-le tel quel, y compris les `\n` et les lignes `-----BEGIN/END PRIVATE KEY-----`, entre guillemets dans Vercel). **Clé serveur uniquement**, jamais exposée côté client. |
| `NEXT_PUBLIC_APP_URL`                        | ⚠️     | URL publique du jeu utilisée dans les liens des e-mails (bouton "Rejoindre le jeu"). Défaut si absent : `https://horizon-zeldcraft.vercel.app`. |

> **Repli en chaîne automatique** : configure autant de ces 4 clés que tu veux (une seule suffit pour activer l'assistant IA). Si le fournisseur choisi par l'admin dans le menu « Intelligence IA GamePlay » dépasse son quota gratuit (erreur 429, cas fréquent chez Gemini), la route API bascule **automatiquement** sur le prochain fournisseur dont la clé est configurée (ordre : Gemini → Groq → Cerebras → OpenRouter), jusqu'à obtenir une réponse ou épuiser la liste. Sans aucune des 4 clés, le bouton « Générer une analyse » répond simplement « non configuré » — le reste du panneau (statistiques, heatmaps, entonnoir de quêtes…) fonctionne normalement. **Important** : ajouter ces variables dans `web/.env.local` (dev) ou dans le tableau de bord Vercel (Settings → Environment Variables → Production) ne suffit pas à lui seul si le code n'était pas déployé avec le repli en chaîne — un redéploiement est nécessaire après tout changement (voir section suivante), et **au moins une clé doit être renseignée pour que le repli fonctionne** (ajouter du code sans jamais ajouter la clé chez le fournisseur ne peut pas résoudre un 429 : chaque fournisseur a son propre quota, indépendant des autres).

> Toutes les clés préfixées `NEXT_PUBLIC_` sont exposées côté navigateur — n'y mets **jamais** de secret sensible (clé privée, mot de passe). Les clés Firebase Web SDK sont publiques par design ; la sécurité repose sur les **règles Firebase RTDB** (voir `FIREBASE_CHAT.md`). `GEMINI_API_KEY`, `GROQ_API_KEY`, `CEREBRAS_API_KEY` et `OPENROUTER_API_KEY` n'ont volontairement **pas** le préfixe `NEXT_PUBLIC_` : elles ne sont lues que côté serveur dans `web/src/app/api/ai/insights/route.ts`.
